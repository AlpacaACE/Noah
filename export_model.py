...
def export_transformers_model():
    ...
    ckpt_path = f'./output/full_sft_dense/full_sft_{lm_config.dim}{moe_path}.pth'
    ...
    lm_model.save_pretrained("Noah", safe_serialization=False)


def export_tokenizer(tokenizer_path):
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path,
                                              trust_remote_code=True, use_fast=False)
    tokenizer.save_pretrained("Noah")

if __name__ == '__main__':
    # 1
    export_transformers_model()
    # 2
    export_tokenizer(tokenizer_path="./tokenizer_mistral")
