...
def train_epoch(epoch, save_steps):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % save_steps == 0 and (not ddp or dist.get_rank() == 0):
            model.eval()
...
def init_model(lm_config):
    ...
    if model_from == 1:
        moe_path = '_moe' if lm_config.use_moe else ''
        ckp = f'./output/pretrain_moe/pretrain_{lm_config.dim}{moe_path}.pth'
...
if __name__ == "__main__":
    ...
    #df = pd.read_csv('./dataset/sft_data_multi-turn.csv') # 修改微调数据集
